...
const generateNewsNumber = (lastNumber?: string): string => {
  ...
  const match = lastNumber.match(/NEWS-(\d+)/);
  if (match) {
    const num = parseInt(match[1], 10);
    const nextNum = num + 1;
    return `NEWS-${nextNum.toString().padStart(3, "0")}`;
  }
...
export default function CreateNewsPage() {
  ...
  useEffect(() => {
    if (newsList && newsList.length > 0) {
      const lastNews = newsList[0];
      const generatedNumber = generateNewsNumber(lastNews.number);
      setFormData((prev) => ({ ...prev, number: generatedNumber }));
    } else {
      setFormData((prev) => ({ ...prev, number: "NEWS-001" }));
    }
  }, [newsList]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let imageUrl = "";
      if (imageStorageId) {
        // We'll use the storage ID, but also need a URL for backward compatibility
        // The image field can be empty if we have storageId
