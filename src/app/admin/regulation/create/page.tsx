...
export default function CreateRegulationPage() {
  ...
  const createMutation = useMutation(api.regulations.create);
  ...
  const regulationsList = useQuery(api.regulations.list, { limit: 1 });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const lastRegulation =
        regulationsList && regulationsList.length > 0
          ? regulationsList[0]
          : null;
      const generatedNumber = generateRegulationNumber(lastRegulation?.number);
