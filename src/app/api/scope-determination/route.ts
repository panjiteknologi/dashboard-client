...
- Do NOT translate the query, only correct typos`;
    ...
    if (directSearchResults.length > 0) {
      ...
      for (const result of directSearchResults) {
        ...
        if (!groupedResults[groupKey]) {
          groupedResults[groupKey] = {
            ...
            iaf_code: iafScope.IAF_CODE,
            nace: { code: naceDetail.NACE.code, description: naceDetail.NACE.nace_description },
            nace_child: { code: naceChild.code, title: naceChild.nace_child_title },
            nace_child_details: [],
      ...
      const explanationGroups: Record<string, {
        standar: string;
        iaf_items: Map<string, { iaf_code: string; nace_items: Map<string, { nace_code: string; nace_description: string; nace_children: Set<string> }> }>;
        max_score: number;
      }> = {};
      ...
      for (const result of resultsToShow) {
        ...
        if (!explanationGroups[groupKey]) {
          explanationGroups[groupKey] = { standar: result.standar || result.scope_key, iaf_items: new Map(), max_score: result.relevance_score };
        }
        const iafKey = result.iaf_code;
        if (!explanationGroups[groupKey].iaf_items.has(iafKey)) {
          explanationGroups[groupKey].iaf_items.set(iafKey, { iaf_code: result.iaf_code, nace_items: new Map() });
        }
        ...
        if (!iafItem.nace_items.has(naceKey)) {
          iafItem.nace_items.set(naceKey, { nace_code: result.nace.code, nace_description: result.nace.description, nace_children: new Set() });
        }
...
- Response MUST be valid JSON with this exact structure:
{
  ...
  "saran": "..."
    ...
    if (!aiResult || !Array.isArray(aiResult.hasil_pencarian)) {
      ...
      return NextResponse.json({ hasil_pencarian: [], penjelasan: errorPenjelasan, saran: errorSaran, query }, { status: 500 });
    }
    ...
    for (const result of aiResult.hasil_pencarian) {
      ...
      if (!actualScopeKey) continue;

      const scopeInfo = scopeData[actualScopeKey];
      const aiIAFCodeNormalized = normalizeString(result.iaf_code);

      const iafScope = scopeInfo.scope.find(s => normalizeString(s.IAF_CODE) === aiIAFCodeNormalized);
      if (!iafScope || !Array.isArray(iafScope.NACE_DETAIL_INFORMATION)) continue;
