...
`;
      ...
      const suggestedQuery =
        typoCheckResponse.candidates?.[0]?.content?.parts?.[0]?.text?.trim();

      if (
        suggestedQuery &&
        suggestedQuery !== query &&
        suggestedQuery.toLowerCase() !== query.toLowerCase()
      ) {
        correctedQuery = suggestedQuery;
        hasCorrected = true;
        console.log(
          `✅ Typo corrected (${correctionLanguage}): "${query}" → "${correctedQuery}"`
        );
      } else {
        console.log(
          `✓ No typo detected in: "${query}" (${correctionLanguage})`
        );
      }
    } catch (typoError) {
      console.error(
        "⚠️ Typo correction failed, using original query:",
        typoError
      );
    }
    ...
    console.log(
      `📂 Using scope data: scope_${isIndonesian ? "id" : "en"}.json`
    );
    ...
    function searchInScopeData(keyword: string) {
      ...
      console.log(
        `🔍 Attempting direct search for: "${trimmedKeyword}" (${isSingleWord ? "single word" : "phrase"})`
      );
      ...
      console.log(
        `🔍 Search tokens after filtering: [${searchWords.join(", ")}]`
      );

      // If no meaningful keywords left after filtering, return empty results
      if (
        searchWords.length === 0 ||
        (searchWords.length === 1 && searchWords[0].length <= 2)
      ) {
        console.log(
          `⚠️ No meaningful keywords found after filtering stop words`
        );
        return results;
      }
      ...
      const searchRegex = isSingleWord
        ? new RegExp(
            `\\b${trimmedKeyword.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")}\\b`,
            "i"
          )
        : null;

      // Create regex for each search word
      const wordRegexes = searchWords.map(
        (word) =>
          new RegExp(
            `\\b${word.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")}\\b`,
            "i"
          ...
          for (const naceDetail of iafScope.NACE_DETAIL_INFORMATION) {
            ...
            for (const naceChild of naceDetail.NACE_CHILD) {
              ...
              for (const childDetail of childDetails) {
                ...
                const naceText =
                  naceDetail.NACE.nace_description?.toLowerCase() || "";
                const naceChildText =
                  naceChild.nace_child_title?.toLowerCase() || "";
                const titleText = childDetail.title?.toLowerCase() || "";
                const descText =
                  childDetail.nace_child_detail_description?.toLowerCase() ||
                  "";

                // Ekstraksi kode murni untuk Direct Search jika keyword adalah kode (mis. "16")
                const extractedIAF =
                  iafText.match(/\(([^()]+)\)/)?.[1]?.trim() || iafText;
                ...
                if (isSingleWord && searchRegex) {
                  ...
                  const isIafCodeExact =
                    extractedIAF === trimmedKeyword.toUpperCase();

                  if (isIafCodeExact || iafText.match(searchRegex)) {
                    iafMatchCount = 1;
                    matched = true;
                  }
                  if (naceText.match(searchRegex)) {
                    naceMatchCount = 1;
                    matched = true;
                  }
                  if (naceChildText.match(searchRegex)) {
                    naceChildMatchCount = 1;
                    matched = true;
                  }
                  if (titleText.match(searchRegex)) {
                    titleMatchCount = 1;
                    matched = true;
                  }
                  if (descText.match(searchRegex)) {
                    descMatchCount = 1;
                    matched = true;
                  }
                  ...
                  if (!matched) {
                    ...
                    if (extractedIAF === trimmedKeyword) {
                      iafMatchCount = 1;
                      matched = true;
                    }
                    if (naceDetail.NACE.code === trimmedKeyword) {
                      naceMatchCount = 1;
                      matched = true;
                    }
                    if (naceChild.code === trimmedKeyword) {
                      naceChildMatchCount = 1;
                      matched = true;
                    }
                    if (childDetail.code === trimmedKeyword) {
                      titleMatchCount = 1;
                      matched = true;
                    }
                  }
    ...
    if (canUseDirectSearch) {
      ...
      console.log(
        `🔍 Direct search found ${directSearchResults.length} matches for "${correctedQuery}"`
      );
    }
    ...
    if (directSearchResults.length > 0) {
      console.log(
        `✅ Using direct search results (${directSearchResults.length} matches)`
      );
      ...
      for (const result of directSearchResults) {
        ...
        const iafScope = scopeInfo.scope.find(
          (s) => s.IAF_CODE === result.iaf_code
        );
        if (!iafScope || !Array.isArray(iafScope.NACE_DETAIL_INFORMATION))
          continue;

        const naceDetail = iafScope.NACE_DETAIL_INFORMATION.find(
          (n) => n.NACE && n.NACE.code === result.nace_code
        );
        if (
          !naceDetail ||
          !("NACE" in naceDetail) ||
          !Array.isArray(naceDetail.NACE_CHILD)
        )
          continue;

        const naceChild = naceDetail.NACE_CHILD.find(
          (nc) => nc.code === result.nace_child_code
        );
        ...
        for (const childDetail of sortedDetails) {
          const alreadyExists = groupedResults[
            groupKey
          ].nace_child_details.some(
      ...
      const detailedResults = Object.values(groupedResults).filter(
        (r) => r.relevance_score >= 20 && r.nace_child_details.length > 0
      );
      detailedResults.sort((a, b) => b.relevance_score - a.relevance_score);

      console.log(
        `📦 Grouped into ${detailedResults.length} result cards (Direct Search)`
      );
      ...
      const topResults = detailedResults
        .filter((r) => r.relevance_score >= 70)
        .slice(0, 15);
      const resultsToShow =
        topResults.length > 0 ? topResults : detailedResults.slice(0, 15);

      // Group results by standar only
      const explanationGroups: Record<
        string,
        {
          standar: string;
          iaf_items: Map<
      ...
      for (const result of resultsToShow) {
        ...
        if (result.nace_child?.code) {
          iafItem.nace_items
            .get(naceKey)!
            .nace_children.add(result.nace_child.code);
        }
    ...
    // =====================================================================================
    console.log(
      `⚠️ Direct results not found or query is complex, falling back to AI semantic search`
    );
...
`;
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (!aiResponseText) {
          ...
          if (
            aiResult.hasil_pencarian &&
            Array.isArray(aiResult.hasil_pencarian)
          ) {
            break; // Success
          }
        }
        ...
        console.error(
          `Error processing AI response on attempt ${attempt + 1}:`,
          error
        );
      }
      attempt++;
    ...
    console.log(
      `📊 AI returned ${aiResult.hasil_pencarian.length} results (Semantic Search)`
    );
    ...
    for (const result of aiResult.hasil_pencarian) {
      ...
      if (!actualScopeKey) {
        console.warn(
          `[Grouping Skip] Scope key '${aiScopeKey}' not found in source data (Case mismatch or invalid key).`
        );
        continue;
      }
      ...
      if (!iafScope || !Array.isArray(iafScope.NACE_DETAIL_INFORMATION)) {
        ...
        console.warn(
          `[Grouping Skip] IAF code '${result.iaf_code}' failed strict normalization matching in scope ${actualScopeKey}.`
        );
        continue;
      }
      ...
      if (
        !naceDetail ||
        typeof naceDetail !== "object" ||
        !("NACE" in naceDetail) ||
        !Array.isArray(naceDetail.NACE_CHILD)
      ) {
        console.warn(
          `[Grouping Skip] NACE detail structure error: ${result.nace_code}`
        );
        continue;
      }
      ...
      if (!naceChild) {
        console.warn(
          `[Grouping Skip] NACE Child code not found: ${result.nace_child_code}`
        );
        continue;
      }
      ...
      const detailResult = childDetails.find(
        (cd) => cd.code === result.nace_child_detail_code
      );
      if (!detailResult) {
        console.warn(
          `[Grouping Skip] AI suggested NACE Detail Code NOT FOUND in source data: ${result.nace_child_detail_code}`
        );
        continue;
      }
    ...
    const detailedResults = Object.values(groupedResults).filter(
      (r) => r.relevance_score >= 50 && r.nace_child_details.length > 0
    );
    detailedResults.sort((a, b) => b.relevance_score - a.relevance_score);

    console.log(
      `📦 Grouped into ${detailedResults.length} result cards (Semantic Search)`
    );
