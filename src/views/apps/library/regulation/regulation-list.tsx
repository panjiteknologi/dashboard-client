...
const RegulationContentV2 = ({
  ...
  view: "grid" | "list";
  selectedRegulation: RegulationType | null;
  onSelectRegulation: (regulation: RegulationType) => void;
  ...
  if (regulations.length > 0) {
    return (
      <div
        className={`grid ${view === "grid" ? "grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4" : "space-y-3"}`}
      >
        {regulations.map((item) => (
          <RegulationCardView
            key={item.id}
            data={item}
            handleClick={() => onSelectRegulation(item)}
            view={view}
    ...
    <div className="flex flex-col md:flex-row gap-6 md:gap-8">
      ...
      <main className="w-full">
        ...
        <header className="md:hidden sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b border-slate-200 p-4">
          ...
          <div>
            <label htmlFor="kategori-regulasi-mobile" className="sr-only">
              Kategori Regulasi
            </label>
            <select
